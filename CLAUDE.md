...
**`BlockRebalanceOnPoll` is unpinned, and that is written down rather than assumed.** The consumer passes it so a partition cannot be taken away in the middle of a batch, and no test fails when it is removed. @MustafaKemalV found that on `CO2.5` and I could not find a discriminating observable either: with the option and without it, a member held inside a written batch sees zero revocations in the window, the group reads `PreparingRebalance` both ways, and the test stays green. The reading, not yet proven, is that a member frozen in a hook cannot rejoin, so a cooperative rebalance stalls whatever the option says, which makes freezing the member the wrong method for pinning it. A test that pins it would keep the consumer polling and make processing merely slow. Until that test exists, a change that deleted the option would go green.
